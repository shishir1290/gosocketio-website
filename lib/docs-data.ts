...
export const API_REFERENCE_DATA = [
  {
    category: "Server Methods",
    items: [
      { name: "sio.New(opts *Options) *Server", desc: "Creates a new Socket.IO v4 server instance. Passing nil uses standard defaults." },
      { name: "ServeHTTP(w, r)", desc: "Implements http.Handler for standard Go multiplexers (http.ServeMux, Chi, Gin, Fiber)." },
      { name: "Serve() error", desc: "Accepts and handles Engine.IO transport sessions in the background." },
      { name: "OnConnect(ns, fn)", desc: "Registers a connection handler for a specific namespace. Return error to reject connection." },
      { name: "OnDisconnect(ns, fn)", desc: "Registers a disconnection handler with reason string." },
      { name: "OnEvent(ns, event, fn)", desc: "Registers a listener for a custom JSON event." },
      { name: "OnBinaryEvent(ns, event, fn)", desc: "Registers a listener for binary events with raw buffer slices." },
      { name: "ToRoom(ns, room, event, skip, args...)", desc: "Broadcasts an event to all members in a room, optionally skipping a connection." },
      { name: "ToNamespace(ns, event, args...)", desc: "Broadcasts an event to all connected clients in a namespace." },
      { name: "Count() int", desc: "Returns the count of currently active sessions." },
      { name: "Close() error", desc: "Gracefully disconnects all active sessions and shuts down the transport." }
    ]
  },
  {
    category: "Conn Interface",
    items: [
      { name: "ID() string", desc: "Returns the unique 128-bit base64 connection session identifier." },
      { name: "Namespace() string", desc: "Returns the normalized namespace string (e.g. '/' or '/chat')." },
      { name: "Emit(event, args...) error", desc: "Sends an event packet to this specific connection." },
      { name: "EmitWithAck(event, fn, args...) error", desc: "Emits an event and executes callback fn when the client acknowledges." },
      { name: "Join(room string)", desc: "Subscribes the connection to a room." },
      { name: "Leave(room string)", desc: "Unsubscribes the connection from a room." },
      { name: "Rooms() []string", desc: "Returns a slice of all room names the connection has joined." },
      { name: "Context() interface{}", desc: "Returns the associated connection context (e.g. auth data, user model)." },
      { name: "SetContext(v interface{})", desc: "Thread-safely stores custom session state." },
      { name: "Close() error", desc: "Closes the connection and leaves all rooms." }
    ]
  }
];
